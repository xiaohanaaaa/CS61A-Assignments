...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        """
        answer=0
        chosed_card=player.hand[0].power(opponent_card)
        for i in range(len(player.hand)):
            if player.hand[i].power(opponent_card)>chosed_card:
                answer=i
                chosed_card=player.hand[i].power(opponent_card)
        self.attack+=player.hand[i].attack
        self.defense+=player.hand[i].defense
        best_card=player.hand[answer]
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if self.attack>=opponent_card.defense or self.defense>=opponent_card.attack:
            self.attack-=1000
            self.sefense-=1000
            re_add=True
